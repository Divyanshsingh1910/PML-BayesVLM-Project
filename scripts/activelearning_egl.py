...
from bayesvlm.vlm_elg import CLIP, EncoderResult, ProbabilisticLogits # <-- Added ProbabilisticLogits import
...
def main(
    ...
    if run_egl:
        ...
        if egl_key not in subset_indices_train:
            # Create copies FIRST
            initial_img_projection = copy.deepcopy(image_encoder.vision_projection).to(device)
            initial_txt_projection = copy.deepcopy(text_encoder.text_projection).to(device)

            # --- >>> Ensure requires_grad is True BEFORE calling select_egl <<< ---
            initial_img_projection.train() # Set to train mode
            for param in initial_img_projection.parameters():
                param.requires_grad_(True)
            # --- >>> End of change <<< ---

            # Ensure text projection is eval mode
            initial_txt_projection.eval()
            for param in initial_txt_projection.parameters():
                param.requires_grad_(False)

                
            indices_egl_test, values_egl_test = select_egl(
                clip=clip,
                img_projection=initial_img_projection, # Pass the modifiable projection
            ...
            # --- >>> Turn grads off AFTER select_egl if needed <<< ---
            initial_img_projection.eval() # Set back to eval if desired
            for param in initial_img_projection.parameters():
                param.requires_grad_(False)
