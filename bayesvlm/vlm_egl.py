...
class CLIP(torch.nn.Module):
    ...
    def __init__(
        self,
        logit_scale: float,
        logit_bias: float = 0, # Keep original init
        source_covariance: KroneckerFactorizedCovariance = None,
        target_covariance: KroneckerFactorizedCovariance = None,
        device: Optional[str] = None,
    ):
        super().__init__()
        # Make sure parameters are created on the correct device from the start
        self.logit_scale = torch.nn.Parameter(torch.tensor(logit_scale, device=device))
        self.logit_bias = torch.nn.Parameter(torch.tensor(logit_bias if logit_bias is not None else 0.0, device=device)) if logit_bias is not None else None
    ...
    @property
    def device(self):
        # Return device from parameter or stored attribute
        if self._device:
            return torch.device(self._device)
        return self.logit_scale.device # Fallback to parameter device
    ...
    def set_covariances(
        ...
        # Ensure device consistency when setting covariances
        target_device = self.device
        self.source_covariance = KroneckerFactorizedCovariance(
            A_inv=source_covariance.A_inv.clone().to(target_device),
            B_inv=source_covariance.B_inv.clone().to(target_device),
        ) if source_covariance is not None else None

        self.target_covariance = KroneckerFactorizedCovariance(
            A_inv=target_covariance.A_inv.clone().to(target_device),
            B_inv=target_covariance.B_inv.clone().to(target_device),
    ...
    def from_huggingface(
        ...
        model = cls(
            logit_scale=clip_model.logit_scale.item(),
            logit_bias=None, # Standard CLIP doesn't use this
            device=device # Pass device to constructor
        )
        # Explicitly move model to device if specified (redundant if constructor handles it, but safe)
        if device is not None:
    ...
    def deterministic_forward(
        ...
        # cosine similarity
        # Ensure logit_scale is on the correct device (should be handled by Parameter)
        logits = torch.matmul(source_embeds, target_embeds.t()) * self.logit_scale.exp()
        if self.logit_bias is not None:
            # Ensure logit_bias is on the correct device
    ...
    def _compute_logits( # Alias for deterministic forward for internal consistency if needed
        ...
        # Ensure inputs are EncoderResult for deterministic_forward call
        source_res = EncoderResult(embeds=source_embeds, activations=None)
        target_res = EncoderResult(embeds=target_embeds, activations=None)
        return self.deterministic_forward(source_res, target_res)
    ...
    def _compute_probabilistic_logits_smith(
       ...
       # ... (rest of the _compute_probabilistic_logits_smith method remains the same) ...
        ...
        # Ensure all tensors used are on the correct device
        target_device = self.device
        source_activations = source_results.activations.to(target_device)
        target_activations = target_results.activations.to(target_device)
        source_embeds = source_results.embeds.to(target_device)
        target_embeds = target_results.embeds.to(target_device)

        # Add bias term to activations if necessary
        ...
        # Covariance factors should already be on the correct device via set_covariances
        source_A_inv = source_covariance.A_inv
        ...
        # Compute norms and clamp for stability
        norm_source = source_embeds**2 + source_diag_cov
        expect_norm_source = torch.clamp(norm_source.sum(dim=-1, keepdim=True), min=1e-12)
        norm_target = target_embeds**2 + target_diag_cov
        expect_norm_target = torch.clamp(norm_target.sum(dim=-1, keepdim=True), min=1e-12)
        ...
        variance_similarity = torch.clamp((term1 + term2) / (expect_norm_source @ expect_norm_target.t()), min=0.0)

        # Apply scaling and bias
        scale = self.logit_scale.exp()
        logits_mean = expected_similarity * scale
        logits_var = variance_similarity * (scale**2) # Diagonal variance
